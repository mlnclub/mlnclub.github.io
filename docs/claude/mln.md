...
- **Interactives** (week 4's systolic array) are self-contained HTML in
  `assets/html/`, iframed from the stub inside `.mln-iframe-wrap` (660px tall) with a
  download link. Week 17 briefly carried two more (speculative decoding, and a
  transformer drawn down to q/k/v) that the meeting didn't use; Chris had them
  pulled the same night, so they live only in history (commit `cd89012`). That
  commit is the reference if another week needs one: the pieces post their rendered
  height via `postMessage` so the stub can grow the iframe (grow-only), and the root
  carries `min-width:0; width:100%` so a flex item of the centered body doesn't clip
  its left edge in a narrow column. Attribution line: "Visualization generated by
  Claude." `assets/html/` itself was left behind in the 2026-09-10 split — week 4's
  embed 404'd until it was carried over on 2026-09-13.
