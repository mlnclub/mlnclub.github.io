...
- **Interactives** (week 4 systolic array, week 17 speculative decoding) are
  ...
  660px tall by default; a taller piece takes an inline `style="height: …px"` on the
  iframe, sized to its tallest *state* measured in the embedded column (≈528px wide
  on desktop), not on the standalone page. Same scoped-component conventions in both
  files (`#mac-viz` / `#spec-viz`: palette vars, Play/Step/Reset, readout). The
