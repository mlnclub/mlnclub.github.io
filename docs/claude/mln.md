...
- **Interactives** (week 4 systolic array; week 17 speculative decoding and its
  companion `transformer-logits.html`, which shares the same toy numbers) are
  self-contained HTML in `assets/html/`, iframed from the stub inside
  `.mln-iframe-wrap` with a download link — copy week 17's stub. The wrap's iframe is
  660px tall by default and the column it sits in ranges from ~340px (phone) to
  ~530px, so the week 17 pieces post their rendered height (`postMessage` from a
  `ResizeObserver`) and a small script at the end of the stub grows the matching
  iframe — grow only, never shrink, so stepping doesn't jitter. The inline
  `style="height: …px"` is just the pre-JS fallback. Each interactive's root also
  carries `min-width:0; width:100%` — as a flex item of the centered body it would
  otherwise refuse to shrink and clip its left edge in a narrow column. Same
  scoped-component conventions in all files (`#mac-viz` / `#spec-viz` / `#tf-viz`:
  palette vars, Play/Step/Reset, readout). The
  attribution line is "Visualization generated by Claude." `assets/html/` was left
  behind in the 2026-09-10 split — week 4's embed 404'd until it was carried over on
  2026-09-13.
